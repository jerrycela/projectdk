...
// 繪製完整 2×2 傳送門（含動畫效果）- 優化版（3 層渲染）
   ...
   * 設計：等距石座 + 懸浮紫色水晶
   */
  drawDungeonHeart2x2(ctx, x, y, variant = 0) {
    const PA = DK.PixelArt;
    const ISO = PA.Isometric;
  ...
  /** 火把 (T) - 牆上的火把（3種火焰變體）*/
  drawTorchTile(ctx, x, y, variant = 0) {
    const PA = DK.PixelArt;
    ...
    if (v === 0) {
      ...
      // Variant 1: 向左傾斜火焰
      PA.pixel(ctx, x + 5, y + 6, '#ff8800');
      PA.pixel(ctx, x + 6, y + 5, '#ffaa00');
      PA.pixel(ctx, x + 7, y + 5, '#ffaa00');
      PA.pixel(ctx, x + 8, y + 6, '#ff8800');
      PA.pixel(ctx, x + 6, y + 6, '#ffcc44');
      PA.pixel(ctx, x + 7, y + 6, '#ffcc44');
      PA.pixel(ctx, x + 7, y + 7, '#ff9922');
      PA.pixel(ctx, x + 8, y + 7, '#ff9922');
    } else {
      // Variant 2: 向右傾斜火焰
      PA.pixel(ctx, x + 7, y + 6, '#ff8800');
      PA.pixel(ctx, x + 8, y + 5, '#ffaa00');
      PA.pixel(ctx, x + 9, y + 5, '#ffaa00');
      PA.pixel(ctx, x + 10, y + 6, '#ff8800');
      PA.pixel(ctx, x + 8, y + 6, '#ffcc44');
      PA.pixel(ctx, x + 9, y + 6, '#ffcc44');
      PA.pixel(ctx, x + 7, y + 7, '#ff9922');
      PA.pixel(ctx, x + 8, y + 7, '#ff9922');
    }
  },
  ...
  /** 寶箱 (C) - 金幣寶箱（3種開啟狀態）*/
  drawChestTile(ctx, x, y, variant = 0) {
    const PA = DK.PixelArt;
    this.drawFloorTile(ctx, x, y, 0);
  ...
  /** 石柱 (L) - 裝飾石柱（3種破損程度）*/
  drawPillarTile(ctx, x, y, variant = 0) {
    const PA = DK.PixelArt;
    this.drawFloorTile(ctx, x, y, 0);
  ...
  /** 骸骨 (S) - 骷髏骨頭（3種擺放方式）*/
  drawSkullTile(ctx, x, y, variant = 0) {
    const PA = DK.PixelArt;
    this.drawFloorTile(ctx, x, y, 0);
